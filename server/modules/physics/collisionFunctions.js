...
function suckywuckycollideuwu(my, n) {
    if (my.type === 'bhole') {
      ...
      n.velocity.x += ((distX/dist)/25)/1.3
      n.velocity.y += ((distY/dist)/25)/1.3
    } else {
      let distX = n.x - my.x
      let distY = n.y - my.y
      let dist = util.getDistance(my, n)
      my.velocity.x += ((distX/dist)/25)/1.3
      my.velocity.y += ((distY/dist)/25)/1.3
    }
}
...
function advancedcollide(my, n, doDamage, doInelastic, nIsFirmCollide = false) {
    ...
    if (component >= diff.length - combinedRadius) { // A simple check
        ...
        if (goahead) {
          ...
          /*let applyPoison = (bullet, tank) => {
  if (bullet.poison&&!tank.poisoned) {
    tank.poisoned = true;
    let poisonloop = () => {
      tank.damageRecieved += bullet.poison[0]
    },
    poisonloopid = setInterval(poisonloop, 1000*bullet.poison[2]);
    setTimeout(() => {
      clearInterval(poisonloopid);
      tank.poisoned = false;
    }, 1000*bullet.poison[1]);
  }
};
applyPoison(my, n);
applyPoison(n, my);
