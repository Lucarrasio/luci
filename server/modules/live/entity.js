...
class Entity {
            ...
            return {
                update: () => {
                    ...
                    if (!active) {
                        ...
                        this.addToGrid();
                        timer = 15;
                        active = views.some(v => v.check(this, 0.6)) || this.alwaysActive || this.isPlayer || this.isBot;
                    }
                },
                check: () => {
                    return active;
    ...
    define(set) {
       ...
       if (set.MESSAGE != null) {
            this.sendMessage = (set.MESSAGE === '') ? undefined : set.MESSAGE;
        }
    ...
    confinementToTheseEarthlyShackles() {
        ...
        if (c.SPECIAL_BOSS_SPAWNS && (this.type === "tank" || this.type === "food") && room.isIn("outb", {
                x: this.x,
                y: this.y
            })) this.kill();
...
//Shoot on death
      ...
      if (gun.shootOnDeath) {
        ...
        if (this.isDead()) {
            ...
            let killText = (notJustFood) ? '' : "You have been ursurped by ",
                ...
                killers.forEach(instance => {
                    ...
                    if (dothISendAText) {
                        if (name=="a Heal Fruit"){
                         instance.sendMessage('You were healed to max health! (Unless you shot it...)'); 
                        } else {
                        instance.sendMessage('You ursurped ' + name + ((killers.length > 1) ? ' (with some help).' : '.'));
                        }
                    }
                });
           ...
           if (this.settings.sendMessage) sockets.sendMessage(this.settings.sendMessage);
